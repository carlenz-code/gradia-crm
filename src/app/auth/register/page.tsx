...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Aquí manejarías los errores de registro
    } finally {
      setIsLoading(false);
    }
  };

  // Manejar registro con proveedores externos (Google, Facebook, Apple)
  const handleSocialRegister = async (provider: string) => {
    try {
      console.log(`Registro con ${provider}`);
      // Aquí iría la lógica para registro con OAuth
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4 sm:gap-0">
            ...
            <p className="text-xs text-gray-400 text-center sm:text-right">
              ¿Ya tienes cuenta?<br />
              <Link href="/auth/login" className="text-[#0785E6] hover:underline">
                Inicia sesión
              </Link>
            </p>
          </div>
          ...
          <div className="flex gap-3 mb-4">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-md px-4 py-3 text-xs placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-[#3B82F6] focus:border-[#3B82F6]"
                required
              />
            </div>
            <div className="flex-1">
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-md px-4 py-3 mb-4 text-xs placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-[#3B82F6] focus:border-[#3B82F6]"
            required
          />
          ...
          <div className="relative">
            <input
              ...
              className="w-full border border-gray-300 rounded-md px-4 py-3 mb-2 text-xs placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-[#3B82F6] focus:border-[#3B82F6] text-black"
              required
              minLength={8}
            />
            <button
          ...
          <div className="flex items-start gap-3 mb-6 sm:mb-8">
            <input
              ...
              onChange={handleInputChange}
              className="mt-1 w-4 h-4 text-[#0785E6] border-gray-300 rounded focus:ring-[#0785E6]"
              required
            />
            <label
              htmlFor="acceptTerms"
            ...
            >
              Acepto los{' '}
              <Link href="/terms" className="text-[#0785E6] hover:underline">
                términos y condiciones
              </Link>{' '}
              y la{' '}
              <Link href="/privacy" className="text-[#0785E6] hover:underline">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full bg-[#0785E6] text-white rounded-lg py-3 sm:py-4 font-medium text-sm shadow-[0_8px_15px_rgba(7,133,230,0.3)] hover:brightness-110 transition ${isLoading ? 'opacity-70 cursor-not-allowed' : ''
              }`}
          >
